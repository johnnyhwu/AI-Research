...
![AI、ML 與 DL 之間的關係示意圖，顯示三者由外而內的包含關係。](img-001)
...
![機器學習演算法分為三大類的示意圖。](img-002)
*Machine Learning 底下的演算法主要分為三種類別*
...
![Machine Learning 與 Traditional Programming 兩種解決問題方式的對照示意圖。](img-003)
...
在本篇文章中，對 Machine Learning 有了初步的認識，了解 Machine Learning 底下的三種類別的演算法，並區分 Machine Learning 與 Traditional Programming 的區別。此文章屬於 AWS ML Foundation 系列文章的第一篇，下一篇文章將會介紹 Machine Learning 中的「模型」以及「訓練的方法」。

```figure-map
[
  {
    "id": "img-001",
    "references_manifest_caption": "AI、ML 與 DL 之間的關係 [source: Nvidia]",
    "why_used": "說明人工智慧、機器學習與深度學習三者的包含關係，帶出深度學習屬於機器學習底下其中一種演算法的概念。",
    "agent_match_hint": "一張同心圓或包含關係示意圖，由外而內依序是 AI、ML、DL 三層。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "Machine Learning 底下的演算法主要分為三種類別",
    "why_used": "支撐説明機器學習演算法可分為 Supervised、Unsupervised、Reinforcement Learning 三大類的段落。",
    "agent_match_hint": "一張分類示意圖，呈現機器學習底下三種學習方式的分支結構。"
  },
  {
    "id": "img-003",
    "references_manifest_caption": "Machine Learning 與 Traditional Programming 的差別 [source: Udacity]",
    "why_used": "支撐説明 Machine Learning 與 Traditional Programming 兩種解決問題方式差異的段落。",
    "agent_match_hint": "一張對照圖，左右兩側分別呈現傳統程式設計與機器學習的資料、規則、輸出流程差異。"
  }
]
```
