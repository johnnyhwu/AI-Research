...
問一個視覺推理問題給現在的多模態大模型（VLM），它多半會回你一大段文字：機油夠不夠、球會掉進哪個桶子、迷宮該怎麼走。問題是，你要怎麼確認它講的是真的？文字答案和圖片之間沒有直接的對應關係，模型也可能是矇對的，你完全看不出它的判斷依據。
...
像 GPT-4o、Gemini 這類頂級模型的文字生成能力已經很成熟，但一碰到需要精準對應像素位置的空間推理任務，問題就浮現了。使用者問一個關於汽車維修或物理路徑的問題，模型給出一長串文字敘述，但這段文字很難精確對應到圖片上的哪個位置。
...
座標點輸出（如 Molmo、MoonDream）計算量小，但只能給點，沒辦法畫出連續的軌跡或形狀，遇到需要框選或連線的任務就束手無策。影像編輯類的做法（如 Nano Banana Pro）視覺效果直觀，問題是它會直接修改原始像素——這是破壞性的，而且容易產生跟原圖無關的幻覺內容。特定任務微調的模型（如 ViLaSR、ThinkMorph）在訓練過的任務上表現不錯，但換一個沒看過的任務類型，例如新的迷宮佈局，準確度就會崩掉，泛化能力差。
...
![消融實驗表格，比較單輪模式下有無加入格線提示，對 sketchVLM 與另一模型在多項任務上的準確度影響。](img-007)
*表 3 — 加入座標網格對不同模型的影響並不一致：sketchVLM 在有網格時表現最好，另一個模型反而在沒有網格時表現更好。（來源：原始論文）*
...
針對不同任務，作者也設計了對應的指令限制：物體計數只能在中心點標數字、不准畫框；零件標註強制使用預先定義好的標籤清單，避免模型亂取名字；迷宮導航則要求模型「先畫圖、再給答案」，讓標註變成模型自己的草稿紙，輔助牠一步步推理，而不只是事後解釋。
...
![單輪與多輪生成流程的對照示意圖，展示同一個 VPCT 樣本在單輪模式下一次輸出所有標註與答案，以及在多輪模式下逐輪產生標註並重複利用先前標註的過程。](img-004)
