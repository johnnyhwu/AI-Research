...
# 拆解 Cerebras 企業知識庫：一個服務人、自動化流程與 AI Agent 的 RAG 系統怎麼設計

Cerebras 在 2026 年 7 月發表了一篇部落格文章，講的是他們內部知識庫系統「Cerebras Knowledge」的架構。這系統每天要處理超過 15,000 次查詢，資料來源橫跨 Slack、Wiki、程式碼倉庫、Jira，甚至晶片線路表（netlist）這種很難想像會跟聊天記錄放在同一套系統裡處理的東西。更麻煩的是，查詢的一方不只是人：自動化流程與 AI agent 也會直接打這套系統。

這個前提其實決定了很多後面的設計。如果使用者只有人類，介面可以做成一問一答的網頁就好；但當呼叫者裡有另一個具備推理能力的 agent，系統就得同時撐起兩種完全不同的互動方式——這點會在後面講到 MCP 介面設計時特別明顯。

這篇筆記整理了我讀完這篇文章、又拿自己手上企業 RAG 系統的實戰經驗回頭對照後的心得。除了文章本身教的技術細節，我覺得更值得記下來的是兩個文章沒有直接講、但討論過程中自己推導出來的判斷：**檢索與推理必須解耦**，以及**這套架構本質上是單輪 pipeline，不是 agentic loop**。這兩點會在文章後半段展開。
...
這一步為什麼重要,原因在 embedding 這個表示法本身的特性:embedding 品質高度依賴輸入文字的形式一致性。一條討論串可能有 2 個人講 3 句話,也可能有 10 個人吵了 50 則訊息還離題,如果直接把這些長短、參與人數差異極大的原始文字拿去 embed,產生的向量在「什麼代表這個文件」這件事上根本無法互相比較。文章裡有個實測數字:討論串正規化成一致格式之後,準確率有顯著提升。這裡文章用的詞是「normalized」而不是「summarized」,強調的重點是讓向量在同一個表示空間裡語意一致,而不只是把文字弄短。這也給了一個蠻通用的判準:如果原始資料形式差異很大,該問的不是「要不要摘要」,而是「有沒有一個固定 schema,能把這些異質資料收斂成一致格式」。
...
> 這裡我想補一個自己實際用過的心得。如果系統已經在做 BM25 加向量的混合檢索,IDF 其實已經隱含在 BM25 內部了,但把 IDF 額外再抽出來當一個獨立的過濾或加權訊號,拿來專門壓低「語意上很接近查詢、但全是廢話詞」的結果,在處理對話型資料時特別好用——像 Slack 這種充滿社交填充語的資料,BM25 排名本身不會主動幫你濾掉那些在向量檢索裡浮上來的廢話,但一個獨立的 IDF 閘門可以額外做這件事,兩者不衝突,可以疊加使用。
...
如果每次查詢都無腦呼叫所有工具,會帶來兩個直接的壞處:不需要的工具也被觸發,拖慢延遲、浪費成本;不相關工具吐出來的雜訊結果,還可能拉低整體排序品質。Planner 存在的第一原理,是把「這次該用哪些工具」從固定規則變成一個動態決策,讓成本跟延遲跟查詢的實際需求成正比。
...
這給了一個值得記住的延伸判斷:如果系統未來想被其他 agent 呼叫使用,就必須把「檢索原語」跟「編排決策」拆成兩層、可以獨立暴露的介面——這其實是「檢索與推理解耦」原則在更高一層(編排層)的重演。
...
這篇文章的核心貢獻,是完整示範了一套窄腰設計,如何在異質的企業資料場景裡,同時解決寫入端的可擴展性與讀取端的檢索品質問題——這個設計原則從資料抽象層、到讀取融合層、到多工具編排層、再到組織 scoping 層,反覆出現了四次,是全篇最值得內化的單一心智模型。

但拿它跟自己現有系統對照過一輪之後,我覺得最重要的收穫並不是文章羅列的技術細節本身(RRF 公式、distillation 流程、bursting 機制這些),而是這些細節共同指向的一個更根本的原則:檢索與推理必須解耦。原有系統以領域為分化維度、每一路檢索都綁定完整推理與摘要流程的設計,乍看合理,實際上同時造成了「跨領域無法共用」跟「新資料類型需要重複開發」兩個問題,而這兩個問題共同的解法,就是把推理徹底移出檢索路徑,只在管線末端的單次 synthesis 統一發生。

同樣重要的是前面梳理出來的架構邊界:文章描述的整套 planner-executor-synthesis 架構,本質上是一個單輪 pipeline,不具備處理「證據不足需要迭代」或「查詢之間存在序列依賴」這類複雜推理需求的能力——因為它平行 fan-out 的設計,隱含了工具彼此獨立、且所需證據在查詢當下即可被完全決定的假設。這代表文章教的所有技術(混合檢索、reranking、資料蒸餾、動態工具選擇),本質上是在回答「如何把單輪檢索做扎實」,而不是「如何做多步 agentic 推理」——後者需要另一層具備狀態管理與動態規劃能力的迴圈架構,文章完全沒有觸及。看清楚這個邊界的價值在於,它清楚劃分了文章技術直接適用的範圍,跟需要另外補強 agentic RAG 相關知識的範圍,避免把單輪 pipeline 的優化誤認為能解決本質上需要多步推理的問題。

最後想留一個閱讀方法論上的心得:技術細節的價值往往不在於直接照搬,而在於用自己的實戰場景逐一去檢驗它背後的第一原理是否成立、以及在哪些邊界條件下會失效——就像投影片檢索裡 RRF 因為訊號源頭不正交而失效的那個案例。這種對照式的閱讀,比單純理解文章本身,能帶來更深刻、也更能直接落地的架構判斷力。
