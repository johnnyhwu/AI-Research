...
[
  {
    "id": "img-001",
    "references_manifest_caption": "機器學習解決問題的五個步驟",
    "why_used": "作為全篇的骨架圖，讓讀者先看到五個步驟的整體流程，再進入第一個步驟的細節。",
    "agent_match_hint": "一張流程示意圖，依序列出定義問題、建立資料集、訓練模型、評估模型、使用模型五個階段。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "機器學習解決問題的第一步：定義問題",
    "why_used": "標示文章正式進入第一個步驟「定義問題」的段落，呼應前面的五步驟流程圖。",
    "agent_match_hint": "與五步驟流程圖同系列的示意圖，特別標示或凸顯第一個階段「定義問題」。"
  },
  {
    "id": "img-003",
    "references_manifest_caption": "機器學習中監督式學習與非監督式學習的差異",
    "why_used": "支撐說明監督式與非監督式學習關鍵差異（有沒有標籤）的段落。",
    "agent_match_hint": "一張左右對照圖，一側是有標籤的監督式學習，另一側是沒有標籤的非監督式學習。"
  },
  {
    "id": "img-004",
    "references_manifest_caption": "機器學習中任務的簡單分類",
    "why_used": "總覽段落中用來收攏回歸、分類、分群三種任務與兩種學習方式之間的歸屬關係。",
    "agent_match_hint": "一張分類樹狀或分支示意圖，把機器學習任務分到監督式與非監督式底下，列出回歸、分類、分群。"
  }
]
```
