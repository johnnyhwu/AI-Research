...
![機器學習解決問題流程的五個步驟示意圖，第三步「訓練模型」被標示出來。](img-001)
*用機器學習解決問題的第三步驟：模型訓練*
...
在「使用機器學習解決問題的五步驟：定義問題」一文中，我們談過要先辨識「問題」屬於哪一種機器學習的任務，才能建立相對應的模型。實際上機器學習的模型百百種，「選模型」本身就是一個不斷嘗試 (Trial-and-Error) 的過程。這裡把它們大致分成三類。
...
![樹狀模型的判斷流程示意圖，從根節點依序以年齡、是否寫程式等條件分支，最後落到職業類別。](img-002)
*樹狀模型*
...
[
  {
    "id": "img-001",
    "references_manifest_caption": "用機器學習解決問題的第三步驟：模型訓練",
    "why_used": "在前言中定位這篇文章在整個五步驟流程中的位置，讓讀者知道「模型訓練」是第三個步驟。",
    "agent_match_hint": "一張流程示意圖，橫向或環狀列出機器學習解決問題的五個步驟，其中第三步「訓練模型」被特別標示。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "樹狀模型",
    "why_used": "把前面用年齡、是否寫程式判斷職業的文字範例，具體畫成樹狀結構，讓讀者看懂樹狀模型的分支邏輯。",
    "agent_match_hint": "一張由上往下分支的樹狀圖，節點是判斷條件（例如年齡是否大於 25、有沒有在寫程式），葉子是職業類別。"
  }
]
```
