...
![一張示意圖，資料輸入已訓練完成的模型後直接輸出預測結果。](img-001)
*模型推論即是直接使用模型進行預測*
...
![一張左右對照圖，比較模型訓練與模型推論兩個階段的流程差異。](img-002)
...
[
  {
    "id": "img-001",
    "references_manifest_caption": "模型推論即是直接使用模型進行預測",
    "why_used": "作為「模型推論的意義」一節的開場視覺，說明資料輸入已訓練好的模型後直接產生預測結果的流程。",
    "agent_match_hint": "一張流程示意圖，資料流入一個已訓練完成的模型方塊後輸出預測結果。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "模型訓練 (Model Training) 與模型推論 (Model Inference)",
    "why_used": "支撐比較模型訓練與模型推論兩階段差異的段落，讓讀者一眼看出兩者流程上的不同。",
    "agent_match_hint": "一張左右並列的對照圖，一側標示 Model Training、另一側標示 Model Inference，各自呈現其流程。"
  }
]
```
