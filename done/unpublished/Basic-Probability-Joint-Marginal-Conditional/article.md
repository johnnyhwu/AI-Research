# 機率基本觀念：Joint、Marginal 與 Conditional

## 前言

機率 (Probability) 是機器學習的地基。不管是 Naive Bayes、語言模型算下一個 token 的分佈，還是評估模型輸出的信心水準，背後都是同一套機率語言。地基沒打好，後面看到公式只能死背。
...
- **Joint Probability（聯合機率）**
  描述「兩個或多個」事件同時發生的機率。A 與 B 是兩個不同的事件，兩者同時發生的機率寫成 P(A ∩ B)。「∩」符號叫做「交集」，白話一點就是「兩個條件都要成立」。以撲克牌來說，假設 A =「抽出一張 6」且 B =「抽出一張紅色的牌」，則 P(A ∩ B) = 2/52（52 張牌裡，同時是 6 又是紅色的只有 2 張）。

- **Conditional Probability（條件機率）**
  描述在某一個事件已經發生的「前提」之下，另一個事件發生的機率，寫成 P(A | B)，唸作「在 B 發生的前提下 A 發生的機率」。以撲克牌來說，B =「抽出紅色的牌」、A =「抽出數字 4 的牌」，則 P(A | B) = 2/26。關鍵在於分母變了：既然已經確定抽到的是紅色牌，考慮範圍就從 52 張縮小到 26 張紅色牌，而這 26 張裡面數字 4 有 2 張。
...
![乘法法則的數學公式，說明條件機率、聯合機率與邊際機率三者之間的關係。](img-001)
*機率中的乘法法則*

乘法法則的價值就在於它把前面三種機率類型整合進同一條式子裡。用文氏圖 (Venn diagram) 來看會更直覺。

![以兩個部分重疊的圓圈組成的文氏圖，分別代表事件 A 與事件 B，重疊區域代表兩者同時發生。](img-002)
*透過文氏圖了解乘法法則*

P(B) 是 B 事件發生的機率，對應圖中的 B 圓圈；P(A) 是 A 事件發生的機率，對應圖中的 A 圓圈。P(A ∩ B) 則是 A 與 B 同時發生的機率，對應兩個圓圈重疊的那一塊。

所以，在 B 已經發生的前提下 A 發生的機率 P(A | B)，就是「A 與 B 同時發生的機率 P(A ∩ B)」除以「B 發生的機率 P(B)」。直覺上就是：已知 B 發生，等於把整個世界縮小成 B 這個圓圈，這時候 A 還能發生的部分只剩下重疊區域，所以拿重疊區域去除以 B 圓圈。

用剛才的撲克牌例子驗算一次會更有感覺。A =「抽出數字 4」、B =「抽出紅色的牌」，P(A ∩ B) = 2/52、P(B) = 26/52，相除之後得到 (2/52) ÷ (26/52) = 2/26，和前面直接數紅色牌算出來的答案一致。
...
最後用乘法法則把三者接了起來：P(A | B) = P(A ∩ B) / P(B)。下一篇文章會補上其他基本觀念，更詳細地說明 Joint Probability 與 Conditional Probability 的差別，以及機率中 AND 與 OR 的概念。

```figure-map
[
  {
    "id": "img-001",
    "references_manifest_caption": "機率中的乘法法則",
    "why_used": "呈現乘法法則的公式本體，是「把三種機率串起來」這一節的主要依據。",
    "agent_match_hint": "一張以數學公式為主的圖，內容為條件機率、聯合機率與邊際機率之間的乘法關係式。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "透過文氏圖了解乘法法則",
    "why_used": "用視覺化的方式說明 P(A)、P(B) 與 P(A ∩ B) 在集合上的對應位置，幫助讀者理解條件機率為什麼是「重疊區域除以 B 圓圈」。",
    "agent_match_hint": "一張文氏圖，兩個標示為 A 與 B 的圓圈部分重疊，重疊區域代表兩事件同時發生。"
  }
]
```
