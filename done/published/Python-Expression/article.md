...
在前一篇文章中，我們介紹了 Google Colaboratory（Colab）這款好用的線上編輯器。它跑在瀏覽器裡，不用在自己電腦上裝 Python，對初學者來說是最沒有負擔的起手式。
...
![Expression 的組成示意圖，標示出算式中的 Value（數值）與 Operator（運算子）兩個部分。](img-001)
*Expression 的概念*
...
![Colab 中執行錯誤程式碼後跳出 SyntaxError 錯誤訊息的畫面。](img-002)
...
在下一篇文章中，將會介紹程式中「變數」（Variable）的概念，讓我們可以把算出來的結果存起來重複使用，而不是每次都重寫一次算式。

```figure-map
[
  {
    "id": "img-001",
    "references_manifest_caption": "Expression 的概念",
    "why_used": "搭配說明 Expression 由 Value 與 Operator 兩種元素組成，讓讀者把「算式」的直覺對應到程式中的名詞。",
    "agent_match_hint": "一張示意圖，把一段算式拆解標註出 Value（數值）與 Operator（運算子）兩個部分。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "程式初學者常見的錯誤：Syntax Error",
    "why_used": "作為 Syntax Error 段落的實例，呈現漏打數字導致語法錯誤時電腦實際顯示的畫面。",
    "agent_match_hint": "一張編輯器或 Colab 的截圖，程式碼下方出現紅色的 SyntaxError 錯誤訊息。"
  }
]
```
