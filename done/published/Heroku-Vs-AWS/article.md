...
![AWS 官方認證體系的總覽圖，列出各個層級與領域的證照名稱。](img-001)
...
![Heroku 網頁版管理介面的畫面，顯示應用程式的管理選項。](img-002)
...
了解 Heroku 之後，接下來可以看同系列的另一篇文章〈在 M1 Mac 上將 Django 部署到 Heroku〉，實際把一個 Django 專案部署上去。

```figure-map
[
  {
    "id": "img-001",
    "references_manifest_caption": "AWS 的相關證照 [source: AWS]",
    "why_used": "佐證「直接使用 AWS 需要大量先備知識」這個論點：AWS 自己就開了多張證照與課程，說明其學習門檻不低。",
    "agent_match_hint": "一張 AWS 官方證照體系的總覽圖，以分層或分類方式列出多張證照的名稱。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "Heroku Dashboard [source: Heroku]",
    "why_used": "呈現 Heroku 提供給開發者的管理介面，說明它如何把 AWS 的複雜度包裝成更好用的操作體驗。",
    "agent_match_hint": "一張 Heroku 網頁後台的介面截圖，畫面上是應用程式的管理頁面。"
  }
]
```
