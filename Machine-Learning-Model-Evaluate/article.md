...
本篇是機器學習入門觀念系列的第六篇。前一篇「使用機器學習解決問題的五步驟：模型訓練」談的是模型怎麼被訓練出來，以及常見的模型種類。這篇要接著談的是：訓練完之後，我們用什麼方式量化模型的表現、Overfitting 是怎麼一回事，以及分類任務與回歸任務各自會用到哪些評估指標。

![機器學習解決問題五步驟的流程示意圖，標示出目前所在的第四個步驟「模型評估」。](img-001)
*用機器學習解決問題的第四步驟：模型評估*

## 模型評估的概念

在前面「建立資料集」的步驟中，我們把手上的資料切成訓練資料與測試資料兩份。訓練模型時只餵訓練資料，評估模型時則刻意換上測試資料，也就是模型從頭到尾都沒看過的那一份。
...
在「使用機器學習解決問題的五步驟：定義問題」一文中提過，常見的機器學習任務分成「分類」與「回歸」兩種。分類任務可以數對錯，但回歸模型輸出的是連續數值，沒有「對」或「錯」可以數，那要怎麼評估？
...
![scikit-learn 中回歸任務常用評估指標的一覽表。](img-002)
*Regression 常用的 Loss Function [source: scikit-learn]*

![scikit-learn 中分類任務常用評估指標的一覽表。](img-003)
...
![機器學習五步驟的流程示意圖，以箭頭標示評估結果不佳時回頭修正前面步驟的循環。](img-004)
*模型評估的結果如果不佳，可能是前面的某個步驟有問題*
...
[
  {
    "id": "img-001",
    "references_manifest_caption": "用機器學習解決問題的第四步驟：模型評估",
    "why_used": "在前言中定位這篇文章在機器學習五步驟中的位置，讓讀者知道「模型評估」是第四步。",
    "agent_match_hint": "與同系列文章一致的五步驟流程示意圖，特別標示或凸顯第四個階段「模型評估」。"
  },
  {
    "id": "img-002",
    "references_manifest_caption": "Regression 常用的 Loss Function [source: scikit-learn]",
    "why_used": "支撐回歸任務評估指標的段落，列出 scikit-learn 中可直接使用的回歸評估指標。",
    "agent_match_hint": "取自 scikit-learn 文件的表格或列表，內容是 regression 類的 scoring 指標名稱（例如 neg_mean_absolute_error、neg_mean_squared_error 等）。"
  },
  {
    "id": "img-003",
    "references_manifest_caption": "Classification 常用的 Loss Function [source: scikit-learn]",
    "why_used": "與上一張圖成對出現，列出 scikit-learn 中分類任務可直接使用的評估指標。",
    "agent_match_hint": "取自 scikit-learn 文件的表格或列表，內容是 classification 類的 scoring 指標名稱（例如 accuracy、f1 等）。"
  },
  {
    "id": "img-004",
    "references_manifest_caption": "模型評估的結果如果不佳，可能是前面的某個步驟有問題",
    "why_used": "在結論中呼應「評估結果不好要回頭檢查前面步驟」的說法，呈現五步驟並非單向流程而是循環。",
    "agent_match_hint": "同樣是五步驟流程示意圖，但畫有回頭的箭頭或迴圈，表示從模型評估退回前面的步驟。"
  }
]
```
